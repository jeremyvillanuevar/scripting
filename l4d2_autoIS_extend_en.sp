...
public OnClientConnected(client)
{
	...
	PrintToServer ("[autoIS] Player Joined, Current alive survivors: %i, SI spawn amout: %i", AliveSurvivors, SpawnSize);
	PrintToServer ("[autoIS] Current SI spawn interval: [%.2f, %.2f]Second", SpawnTimeMin, SpawnTimeMax);
	CPrintToChatAll ("Con la llegada de un nuevo aliado necesitamos apoyarnos y estar juntos!!");
...
public OnClientDisconnect(client)
{
	...
	//CPrintToChatAll ("{green}[autoIS] {lightgreen}Player Joined, Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
	//CPrintToChatAll ("{green}[autoIS] {lightgreen}Current SI spawn interval: {olive}[%.2f, %.2f]{lightgreen} Second", SpawnTimeMin, SpawnTimeMax);
	CPrintToChatAll ("Compañeros deben ir y otros regresan, el apoyo sigue entre nosotros!!");
}
...
SetCvars()
{
	if (Enabled)
	{
		...
		SetConVarFloat(hSILimitMax, float(MAX_INFECTED));//z_max_player_zombies
		...
		SetBossesCvar();//director_no_bosses
...
CalculateSpawnTimes()
{
	...
	else //if SILimit is <=1 then constant spawn time
		SpawnTimes[0] = SpawnTimeMax;
	#if DEBUG_TIMES
...
public Action:SpawnWitchAuto(Handle:timer, any:waitTime)
{
	...
	if (LeftSafeRoom)
	{
		...
		if (client)
		{
			...
			#if DEBUG_SPAWNS
				LogMessage("[AIS] Spawned Witch Auto");
			#endif
