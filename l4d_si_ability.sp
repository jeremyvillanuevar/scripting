...
public void OnPluginStart()
{
	...
	ConVar cVar = CreateConVar("l4d_si_ability_enabled", "1", "Enable/Disable the Special Infected Slap/Shove Ability Plugin.", FCVAR_DONTRECORD, true, 0.0, true, 1.0);
	g_bCvarEnabled = cVar.BoolValue;
	cVar.AddChangeHook(OnCvarChange_Enabled);

	cVar = CreateConVar("l4d_si_ability_power", "150", "How much force is applied to the victim (Slap ability).", FCVAR_DONTRECORD, true, 0.0);
	g_fCvarPower = cVar.FloatValue;
	cVar.AddChangeHook(OnCvarChange_Power);

	cVar = CreateConVar("l4d_si_ability_vertical_mult", "1.5", "Vertical force multiplier (Slap ability).", FCVAR_DONTRECORD, true, 0.0);
	g_fCvarZMult = cVar.FloatValue;
	cVar.AddChangeHook(OnCvarChange_ZMult);

	cVar = CreateConVar("l4d_si_ability_cooldown_slap", "1.0", "0=Off, >0: Seconds before SI can slap again.", FCVAR_DONTRECORD, true, 0.0);
	g_fCvarAbilityCooldown[Ability_Slap] = cVar.FloatValue;
	cVar.AddChangeHook(OnCvarChange_SlapCooldown);

	cVar = CreateConVar("l4d_si_ability_cooldown_shove", "1.0", "0=Off, >0: Seconds before SI can shove again.", FCVAR_DONTRECORD, true, 0.0);
	g_fCvarAbilityCooldown[Ability_Shove] = cVar.FloatValue;
	cVar.AddChangeHook(OnCvarChange_ShoveCooldown);

	cVar = CreateConVar("l4d_si_ability_announce", "1", "0=Off, 1=Chat, 2=Center chat, 3=Hint.", FCVAR_DONTRECORD, true, 0.0, true, 3.0);
	g_iCvarAnnounce = cVar.IntValue;
	cVar.AddChangeHook(OnCvarChange_Announce);

	cVar = CreateConVar("l4d_si_ability_incap", "68", "Slapping incapacitating people. Add numbers together: 0=Off, 2=Smoker, 4=Boomer, 8=Hunter, 16=Spitter, 32=Jockey, 64=Charger, 126=All. Default: Boomer|Charger.", FCVAR_DONTRECORD, true, 0.0, true, 126.0);
	g_iCvarIncapFlags = cVar.IntValue;
	cVar.AddChangeHook(OnCvarChange_IncapFlags);

	cVar = CreateConVar("l4d_si_ability_slap", "68", "Special Infected who can slap. Add numbers together: 0=Off, 2=Smoker, 4=Boomer, 8=Hunter, 16=Spitter, 32=Jockey, 64=Charger, 126=All. Default: Boomer|Charger.", FCVAR_DONTRECORD, true, 0.0, true, 126.0);
	g_iCvarFlags[Ability_Slap] = cVar.IntValue;
	cVar.AddChangeHook(OnCvarChange_SlapFlags);

	cVar = CreateConVar("l4d_si_ability_shove", "18", "Special Infected who can shove. Add numbers together: 0=Off, 2=Smoker, 4=Boomer, 8=Hunter, 16=Spitter, 32=Jockey, 64=Charger, 126=All. Default: Smoker|Spitter.", FCVAR_DONTRECORD, true, 0.0, true, 126.0);
