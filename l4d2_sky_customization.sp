...
public OnPluginStart()
{
	...
	hRagdollCorpsesEnabled = CreateConVar("sky_ragdoll_survivors", "0", "Enable Survivor ragdoll corpses", FCVAR_NOTIFY);
...
public OnPlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (L4DTeam:GetClientTeam(client) == TEAM_INFECTED && L4D2ZombieClassType:L4D2_GetPlayerZombieClass(client) == ZOMBIECLASS_TANK)
	{
	...
	{
		if (client <= 0 || client > MaxClients || !IsClientInGame(client) || L4DTeam:GetClientTeam(client) != TEAM_SURVIVORS || !IsPlayerAlive(client))
			return;
