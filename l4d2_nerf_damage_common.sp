...
public void BuildMaps()
{
    ...
    // Weapons
    g_smWeaponIDs = CreateTrie();
    g_smWeaponIDs.SetValue("weapon_pistol",            L4D2_WEPID_PISTOL);
    g_smWeaponIDs.SetValue("weapon_smg",               L4D2_WEPID_SMG);
    g_smWeaponIDs.SetValue("weapon_pumpshotgun",       L4D2_WEPID_PUMPSHOTGUN);
    g_smWeaponIDs.SetValue("weapon_autoshotgun",       L4D2_WEPID_AUTOSHOTGUN);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
    ...
    if (GetClientTeam(attacker) != TEAM_SURVIVOR && GetClientTeam(attacker) != TEAM_HOLDOUT)
        return Plugin_Continue;
