...
{
	LoadTranslations("voteblocker.phrases");
		...
		if (GetClientTeam(client) == 1 && !IsClientAdmin(client) && GetConVarBool(g_hSpectatorVote))
		{
			...
			for (new x = 1; x <= MaxClients; x++)
			{
				if (IsClientInGame(x) && !IsFakeClient(x))
				{
					if (client == x)
					{
						PrintToChat(x,"%t","VoteBlock_Spectate", PREFIX);
					}

					else if (IsClientAdmin(x))
					{
						PrintToChat(x,"%t","VoteBlock_TrySpectate", PREFIX, g_sCallerName[client]);
					}

					else
					{
						PrintToChat(x,"%t","VoteMode_BlockSpectator", PREFIX);
					}
				}
			}
			...
			if (GetConVarInt(g_hBlockCount) > 0)
			{
				...
				{
					PrintToChatAll("%t","VoteBlock_KickSpam", PREFIX, g_sCallerName[client]);
					char Message[64];
					FormatEx(Message, sizeof(Message), "%T", "VoteBlock_KickSpam_Popup", client);
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x,"%t","VoteBlock_BlockKickUnallowed", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x,"%t","VoteBlock_BlockKickStarted", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x,"%t","VoteBlock_BlockKickSucessful", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x,"%t","VoteBlock_BlockChangediffUnallowed", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x,"%t","VoteBlock_BlockChangediffStarted", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x,"%t","VoteBlock_BlockChangediffSucessful", PREFIX);
						}
					}
				}
