    ...
    switch (type)
    {
        case TYPE_GASCAN:
        {
            ...
            if (g_iCvar_Gascan & FLAG_GASCAN)
                CreateProp(vPos, client, TYPE_GASCAN);

            if (g_iCvar_Gascan & FLAG_FUEL_BARREL)
                CreateProp(vPos, client, TYPE_FUEL_BARREL);

            if (g_iCvar_Gascan & FLAG_PROPANECANISTER)
                CreateProp(vPos, client, TYPE_PROPANECANISTER);

            if (g_iCvar_Gascan & FLAG_FIREWORKS_CRATE)
                CreateProp(vPos, client, TYPE_FIREWORKS_CRATE);
        }
...
public void CreateProp(float vPos[3], int client, int type)
{
    int entity;
