...
1.0.5 (13-February-2021)
    - Fixed a wrong spawn position to warn commons. (thanks to "Beatles" and "Maur0" for reporting)

1.0.4 (26-January-2021)
    - Added multiple cvars to configure which weapons should warn zombies. (thanks "Sony Arizona" for requesting)
    - Added L4D1 support for silencer upgrade.

1.0.3 (09-January-2021)
    - Added cvar to control if zombies should be warned while a tank is alive. (thanks "Mi.Cura" for requesting)
    - Added cvar to control if zombies should be warned while in starting safe area.
...
#define PLUGIN_DESCRIPTION            "Warns common infected when shooting"
...
static int    gc_iGoalEntRef[MAXPLAYERS+1];
...
public void OnPluginStart()
{
    ...
    g_hCvar_SafeArea                  = CreateConVar("l4d_shot_warns_common_safe_area", "1", "Allow trigger zombies while in the starting safe area.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_TankAlive                 = CreateConVar("l4d_shot_warns_common_tank_alive", "1", "Allow trigger zombies while a tank is alive.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_UpgradeSilencer           = CreateConVar("l4d_shot_warns_common_upgrade_silencer", "1", "Don't trigger zombies while silencer upgrade is active.\nL4D1 only.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    ...
    g_hCvar_SMG                       = CreateConVar("l4d_shot_warns_common_smg", "1", "Trigger common zombies while firing with a SMG.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    ...
    g_hCvar_SMG.AddChangeHook(Event_ConVarChanged);
    ...
    // Admin Commands
    RegAdminCmd("sm_forcechase", CmdForceChase, ADMFLAG_ROOT, "Force commons chase on self (no args) or specified targets. Only works for survivors. Example: self -> sm_forcechase / target -> sm_forcechase @bots");
...
public void OnPluginEnd()
{
    ...
    while ((entity = FindEntityByClassname(entity, CLASSNAME_INFO_GOAL_INFECTED_CHASE)) != INVALID_ENT_REFERENCE)
    {
        if (GetEntProp(entity, Prop_Data, "m_iHammerID") == -1)
        {
            ...
            if (StrEqual(targetname, "l4d_shot_warns_common"))
...
public void GetCvars()
{
    ...
    g_bCvar_SMG = g_hCvar_SMG.BoolValue;
...
public Action CmdPrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "l4d_shot_warns_common_smg : %b (%s)", g_bCvar_SMG, g_bCvar_SMG ? "true" : "false");
