...
public void OnNextFrameMapStarted()
{
    if (!g_bMapStarted)
    {
        RequestFrame(OnNextFrameMapStarted);
        return;
    }
...
public void OnWeaponSwitchPost(int client, int weapon)
{
    ...
    if (modelIndex == g_iModel_Gnome)
    {
        ...
        for (int i = 1; i <= MaxClients; i++)
        {
            ...
            CPrintToChat(i, "%T", "Equipped a gnome", i, client);
        }
    ...
    if (modelIndex == g_iModel_Cola)
    {
        ...
        for (int i = 1; i <= MaxClients; i++)
        {
            ...
            CPrintToChat(i, "%T", "Equipped a cola", i, client);
        }
